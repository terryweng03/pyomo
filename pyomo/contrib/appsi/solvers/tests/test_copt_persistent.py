...
from pyomo.contrib.appsi.solvers.copt import Copt
...
class TestCoptPersistentSimpleLPUpdates(unittest.TestCase):
    def setUp(self):
...
class TestCoptPersistent(unittest.TestCase):
    ...
    def test_linear_constraint_attr(self):
        ...
        opt = Copt()
        opt.set_instance(m)
        opt.set_linear_constraint_attr(m.c, 'UB', 10)
        self.assertEqual(opt.get_linear_constraint_attr(m.c, 'LB'), 1)
        self.assertEqual(opt.get_linear_constraint_attr(m.c, 'UB'), 10)

    def test_quadratic_constraint_attr(self):
        m = pyo.ConcreteModel()
        m.x = pyo.Var()
        m.y = pyo.Var()
        m.c = pyo.Constraint(expr=m.y >= m.x**2 + 10)

        opt = Copt()
        opt.set_instance(m)
        self.assertEqual(opt.get_quadratic_constraint_attr(m.c, 'RHS'), -10)
    ...
    def test_var_attr(self):
        ...
        opt = Copt()
        opt.set_instance(m)
        # COPT does not support 'Start' via setattr; test 'Obj' instead
        opt.set_var_attr(m.x, 'Obj', 1)
        self.assertEqual(opt.get_var_attr(m.x, 'Obj'), 1)
    ...
    #         if cb_where == coptpy.COPT.CBCONTEXT_MIPSOL:
    ...
    def test_solution_number(self):
        ...
        opt = Copt()
        res = opt.solve(m)
        num_solutions = opt.get_model_attr('PoolSols')
        self.assertGreaterEqual(num_solutions, 1)
        ...
        self.assertAlmostEqual(pyo.value(m.obj.expr), 7.607295680844688)
    ...
    def test_zero_time_limit(self):
        ...
        opt.config.load_solution = False
        res = opt.solve(m)
        num_solutions = opt.get_model_attr('hassol')
...
class TestManualModel(unittest.TestCase):
    ...
    def test_node_limit_term_cond(self):
        opt = Copt()
        opt.copt_options.update({"NodeLimit": 1})
